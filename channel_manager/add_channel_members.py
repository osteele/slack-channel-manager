...
def add_channel_members(member_channels_csv, channel_limit, dry_run, verbose):
  ...
  required_headers = {'Email', 'Member'}
  if not set(df.columns) & required_headers:
    die(f"{member_channels_csv.name} must include at least one of {' and '.join(required_headers)}")
  ...
  unregistered_invitees = {em
    for em in df.Email
    if em not in users_by_email}
  if unregistered_invitees:
    print('Invitees who are not in Slack:', ', '.join(unregistered_invitees))
