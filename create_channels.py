...
for _, row in channels_pd.iterrows():
  ...
  channel, action = find_or_create_channel(channel_name, purpose=row.get('Purpose'), topic=row.get('Topic'))
  channel_ids_df = channel_ids_df.append({'Name': channel_name, 'ID': channel['id']}, ignore_index=True)
